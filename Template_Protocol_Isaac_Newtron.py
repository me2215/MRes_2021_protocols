...
def run(protocol:protocol_api.ProtocolContext):
  def IN_assembly_transformation(prom_utr):
    ...
    reservoir_const = protocol.load_labware('4ti0136_96_wellplate_2200ul', 6) # constant reservoir = all parts that are consistent between constructs = destination plasmid in assembly mix + ligase buffer (A1) and 5U-CDS (A2).
    ...
    PCR_plate = tc_mod.load_labware('4ti0960rig_96_wellplate_200ul')
    spare_plate = protocol.load_labware('4ti0960rig_96_wellplate_200ul', 9) #To hold the rest of the final assembly that we're not transforming. 
    #Pipettes.
  ...
  IN_assembly_transformation(prom_utr = prom_utr) #Define the global variable using the tuple generated by our app (GUI_Isaac_Newtron.py). 